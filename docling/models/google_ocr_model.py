...
class GoogleOcrModel(BaseOcrModel):
    def __init__(self, enabled: bool, options: GoogleOcrOptions):
        ...
        if self.enabled:
            install_errmsg = (
                "Error Google Ocr is not enabled"
            )
            ...
            try:
                ...
                if self.options.google_ocr_config_file_path is None:
                    install_errmsg = ("Google OCR Config File is not found on Environment Variable. Please state it "
                                              "in the form of GOOGLE_CONFIG_FILE_PATH")
                    raise ImportError(install_errmsg)

                google_creds = service_account.Credentials.from_service_account_file(self.options.google_ocr_config_file_path)
                self.reader = vision.ImageAnnotatorClient(credentials=google_creds, client_options=client_options)
                ...
                with TimeRecorder(conv_res, "ocr"):
                    ...
                    try:
                        ...
                        for ocr_rect in ocr_rects:
                            ...
                            google_response =  self.reader.text_detection(image=new_image, image_context=self.image_context)
                                ...
                                for block in file_page.blocks:
                                    for paragraph in block.paragraphs:
                                        for word in paragraph.words:
                                            ...
                                            left = (min(box[0].x, box[1].x, box[2].x, box[3].x) / self.scale) + ocr_rect.l
                                            bottom = (max(box[0].y, box[1].y, box[2].y, box[3].y)/ self.scale) + ocr_rect.t
                                            top = (min(box[0].y, box[1].y, box[2].y, box[3].y)/ self.scale) + ocr_rect.t
                                            right = (max(box[0].x, box[1].x, box[2].x, box[3].x) / self.scale) + ocr_rect.l
